...
#shell.prefix("module load java/1.8.0_192; module load python/3.7; module load gcc/7.3, module load r/3.6.1")
localrules: check_merge, split_contigs_list
...
# snakemake --snakefile VariantCalling.sm --cluster-config VariantCalling.clusterconfig.yaml --cluster "sbatch -N 1 -c {cluster.n} --mem={cluster.pmem} --time={cluster.time} --account=rrg-stothard-ac" --rerun-incomplete --printshellcmds -j 20

#########################################################################################
# Path to config file
#########################################################################################
configfile: 'VariantCalling.config.yaml'
...
list_of_samples = get_sample_names(samples)
...
rule final:
    input:
        ...
        expand(FASTQC_DIR + "{sample}_R1_part1.trimP_fastqc.zip", sample=samples),
        expand(ALIGN_DIR + "{sample}.m.bam", sample=samples),
        expand(ALIGN_DIR + "{sample_list_val}_correct.txt", sample_list_val=list_of_samples),
...
rule fastqc_raw:
    ...
    params:
        threads = THREADS
    shell: 
        "{FASTQC} -t {params.threads} --outdir {FASTQC_DIR} {input}"
...
rule split:
    ...
    params:
        ...
        mem = JAVA_MEM
    shell:
        "partition.sh {params.mem} in={input.r1} in2={input.r2} out={params.r1}%.fastq.gz \
...
rule trimmomatic:
    ...
    shell:
        "java -jar {params.mem} {trimmomaticPE} PE -threads {params.threads} \
                ...
                AVGQUAL:20 MINLEN:35 -phred33 -summary {log}"
...
rule fastqc_trimmed:
    input:
        TRIMMED_DATA + "trimmomatic/{sample}_R1_part1.trimP.fastq.gz",
        TRIMMED_DATA + "trimmomatic/{sample}_R2_part1.trimP.fastq.gz"
    output:
        FASTQC_DIR + "{sample}_R1_part1.trimP_fastqc.zip",
        FASTQC_DIR + "{sample}_R2_part1.trimP_fastqc.zip"
    params:
        threads = THREADS
    shell: 
        "{FASTQC} -q -t {params.threads} --outdir {FASTQC_DIR} {input}"
...
rule bwa_map:
    ...
    shell:
        """
        {BWA} mem -M -R '{params.RG_label}' -t {params.threads} \
...
rule sam_to_sorted_bam:
    ...
    params:
        javamem = JAVA_MEM
    shell:
        "java {params.javamem}  -jar {PICARD} SortSam \
...
rule merge_bam:
    ...
    params:
        ...
        javamem = JAVA_MEM
    run:
        shell("java {params.javamem}  -jar {PICARD} MergeSamFiles {params.bam} O={output}")
...
rule agg_and_dedup:
    ...
    shell:
        """
        java {params.javamem} -jar {PICARD} MarkDuplicates \
            {params.input_line} \
            O={output.mdbam} \
...
rule check_merge:
    input:
        merge = ALIGN_DIR + "{sample_list_val}.m.md.bam"
    params:
        sample_val = "{sample_list_val}"
    output:
        merge_check = ALIGN_DIR + "{sample_list_val}_correct.txt"
    script:
        """
        scripts/bam_checker.py 
        """
...
rule build_bai:
    ...
    shell:
        """
        java {params.javamem} -jar {PICARD} BuildBamIndex I={input} O={output}
        """
...
rule counts1_for_BQSR:
    ...
    shell:
        """
        {JAVA_1P8} {params.javamem} -jar {GATK} \
            -T BaseRecalibrator \
            -I {input.bam} \
            -nct {params.threads} \
            -R {params.reference} \
...
rule haplotype_caller_contigs:
    ...
    output:
        expand(GVCF_DIR + "contigs/{{sample}}.{{contigs_interval}}.g.vcf.gz")
    params:
        javamem = JAVA_MEM,
        tmpdir =  "./tmp",
...
rule genotype_contigs:
    ...
    params:
        tmpdir = "./tmp" ,
        javamem = JAVA_MED +  " -Djava.io.tmpdir=./tmp",
        threads = THREADS_BIG,
        reference = REFERENCE + ".fa",
...
rule cat_snps:
    ...
    shell:
        """
        {JAVA_1P8} -cp  {GATK} org.broadinstitute.gatk.tools.CatVariants \
            -R {params.reference} \
            -V {params.variants} \
            -out {output} \
        """
